...
buffer_t main_buf = {0, 0, 0, PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER, PTHREAD_COND_INITIALIZER};
buffer_t reverse_buf = {0, 0, 0, PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER, PTHREAD_COND_INITIALIZER};
args_t temp_args = {0};
...
void write_item(buffer_t* cb, Item* buf, Item* item) {
    ...
    // TODO: controllo da rimuovere
    if (cb->count >= BUFFER_SIZE) { perror("Overflow sul buffer"); }
